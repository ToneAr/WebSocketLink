...
$tlsRootDirectory =
	ExpandFileName @ FileNameJoin[{DirectoryName[$InputFileName], "..", ".."}];
...
initializeTLS[] :=
	Module[{
		...
		hasRequiredOpens =
			TrueQ @ Quiet @ Check[WebSocketTLS`hasRequiredModuleOpens[], False];
		If[version =!= $tlsHelperVersion || !hasRequiredOpens, restarted = True;
		JLink`ReinstallJava[
			JLink`JVMArguments -> $tlsJVMArguments,
			JLink`ClassPath    -> {$tlsJarPath}
		];
		...
		hasRequiredOpens =
			TrueQ @ Quiet @ Check[WebSocketTLS`hasRequiredModuleOpens[], False];
		If[version =!= $tlsHelperVersion || !hasRequiredOpens, Return[
			Failure[
				"WebSocketLink`TLS",
				<|
					"MessageTemplate"   -> StringJoin[
						"Loaded TLS helper version `` with ",
						"required module opens = `` does not ",
						"match expected version `` from ``."
					],
					"MessageParameters" -> {
						version,
						hasRequiredOpens,
		...
		If[restarted, $tlsDefaultKeyStore = None;
		$tlsDefaultPassword = "";];
		$tlsInitialized = True;
		True
	];
...
makeTLSServerContext[certConfig_] :=
	Module[{initResult, ks, password},
		initResult = initializeTLS[];
			...
			Return[
				Failure[
					...
					<|
						"MessageTemplate" ->
							StringJoin[
								"Failed to resolve TLS server certificate ",
								"configuration."
							]
					|>
				]
...
makeTLSClientContext[verifyPeer_, certConfig_ : Automatic] :=
	Module[{initResult, ctx},
		initResult = initializeTLS[];
		If[FailureQ[initResult], Return[initResult]];
		ctx =
			Which[
				...
				certConfig === Automatic &&
				TrueQ[JLink`JavaObjectQ[$tlsDefaultKeyStore]],
					WebSocketTLS`createClientSSLContextWithTrustStore[
						$tlsDefaultKeyStore
					],
				True,
 ...
 *)
StartTLSServerProxy[externalPort_Integer, certConfig_] :=
	Module[{
			initResult,
			sslCtx,
			sslServerSocket,
		...
		If[FailureQ[sslCtx] || sslCtx === $Failed, Return[sslCtx]];
		sslServerSocket =
			WebSocketTLS`createSSLServerSocket[sslCtx, externalPort];
		If[
			sslServerSocket === $Failed ||
			!TrueQ[JLink`JavaObjectQ[sslServerSocket]],
			Return[
				Failure[
					"WebSocketLink`TLS",
					<|
		...
		If[!IntegerQ[loopbackPort], Return[
			Failure[
				"WebSocketLink`TLS",
				<|
					"MessageTemplate" ->
						StringJoin[
							"Failed to allocate a loopback port for the ",
							"TLS server proxy."
						]
				|>
			]
...
StartTLSClientProxy[
	...
	Module[{
		...
		If[
			loopbackServer === $Failed ||
			!TrueQ[JLink`JavaObjectQ[loopbackServer]],
			Return[
				Failure[
					"WebSocketLink`TLS",
					<|
